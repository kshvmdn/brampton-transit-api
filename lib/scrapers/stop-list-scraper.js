  ...
  static _parse (html, filters = {}) {
    return new Promise((resolve, reject) => {
      ...
      let responses = (routes.find('option').slice(1).map((i, route) => {
        ...
        if ((filters.routeNumber && filters.routeNumber !== routeNumber) || (filters.routeName && !routeName.includes(filters.routeName))) {
          return
        }
        ...
        return new Promise((rResolve, rReject) => { /* eslint promise/param-names: 0 */
          ...
          StopListScraper._fetch({ payload }, (err, res) => {
            ...
            let parsedStops = (stops.find('option').slice(1).map((i, stop) => {
              ...
              if ((filters.stopCode && filters.stopCode != stopCode) || (filters.stopName && !stopName.includes(filters.stopName))) { /* eslint eqeqeq:0 */
                return
              }
